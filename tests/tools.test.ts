...
test('工具齐备，名称与契约一致', () => {
  const { byName } = setup()
  assert.deepEqual([...byName.keys()].sort(), ['forget', 'memory_audit', 'memory_browse', 'memory_check', 'memory_health', 'memory_relate', 'memory_stats', 'memory_version', 'recall', 'remember', 'update'])
